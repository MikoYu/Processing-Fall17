...
int scl = 3;  // Scaling down the video
...
void setup() {
  ...
  movShow = new OpenCV(this, width, height);
  movShow.loadCascade(OpenCV.CASCADE_FRONTALFACE); 
...
void draw() {
  movShow.loadImage(mov);
  movShow.brightness(0);
  image(movShow.getOutput(), 0, 0, width, height);
